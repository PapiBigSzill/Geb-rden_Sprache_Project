...
D_SCHWELLE_GROSSE_FINGER = 0.05
D_SCHWELLE_KLEINER_FINGER = 0.03
D_SCHWELLE_DAUMEN = 0.20
...
def get_finger_status(original_hand_landmarks, frame):
    """
    ...
    aktueller_code = ""
    text_y_start = 150
    x0 = original_hand_landmarks.landmark[0].x
    y0 = original_hand_landmarks.landmark[0].y

    DAUMEN_RELATIVE_DISTANZ_SQR = 0.007

    FINGER_DATA = [
        (4, 2, D_SCHWELLE_DAUMEN, "Daumen"),
        (8, 5, D_SCHWELLE_GROSSE_FINGER , "Zeigefinger"),
        (12, 9, D_SCHWELLE_GROSSE_FINGER, "Mittelfinger"),
        (16, 13, D_SCHWELLE_GROSSE_FINGER , "Ringfinger"),
        (20, 17, D_SCHWELLE_KLEINER_FINGER, "Pinky")
    ]
    for i, (tip_id, base_id, schwelle, name) in enumerate(FINGER_DATA):
        xt = original_hand_landmarks.landmark[tip_id].x
        yt = original_hand_landmarks.landmark[tip_id].y
        xb = original_hand_landmarks.landmark[base_id].x
        yb = original_hand_landmarks.landmark[base_id].y

        # --- SPEZIELLE LOGIK FÜR DEN DAUMEN (i = 0) ---
        if i == 0:
            # Daumen (4 vs 2): Misst die reine Länge des Daumens
            distanz_tip_base_sqr = (xt - xb) ** 2 + (yt - yb) ** 2

            if distanz_tip_base_sqr > DAUMEN_RELATIVE_DISTANZ_SQR:
                status = "1"
            else:
                status = "0"

        # --- ALLGEMEINE LOGIK FÜR LANGE FINGER ---
        else:
            # Für Zeigefinger, Mittel-, Ring- und Kleinfinger: Distanz zu P0
            distanz_tip_sqr = (xt - x0) ** 2 + (yt - y0) ** 2
            distanz_base_sqr = (xb - x0) ** 2 + (yb - y0) ** 2

            # WICHTIG: Verwende hier die korrekt kalibrierten, kleinen Schwellen (z.B. 0.05)
            if distanz_tip_sqr > distanz_base_sqr * (1 + schwelle):
                status = "1"
            else:
                status = "0"

        # --- STATUS-ANZEIGE (Bleibt gleich) ---
        if status == "1":
            color = (0, 255, 255)
    ...
    cv2.putText(frame, f"Code: {aktueller_code}", (50, 400),
...
def draw_status_text(frame, erkannte_zahl, erkannter_buchstabe, winkel_anzeige, aktueller_code):
    ...
    elif erkannter_buchstabe == "v":
        text_symbol = f"Symbol: V, {winkel_anzeige}"
        color_symbol = (0, 0, 255)
    else:
        text_symbol = "Symbol: ?"
...
while True:
    ...
    if results_hands.multi_hand_landmarks:
        ...
        for hand_landmarks in results_hands.multi_hand_landmarks:
            ...
            aktueller_code = get_finger_status(hand_landmarks, frame)
